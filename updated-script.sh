...
# Create EKS cluster
eksctl create cluster --name test-ai-app1 --version 1.30 --region us-west-2 --nodegroup-name standard-workers --node-type t3.medium --nodes 2 --nodes-min 1 --nodes-max 3 --managed

# Get the role name from the EKS node group
ROLE_NAME=$(aws eks describe-nodegroup \
    --cluster-name test-ai-app1 \
